...
void C_ENEMY::RespawnCalc()
{
	...
	//{
	//	m_objectData.pos = m_startPos;
	//	m_hp = 1;
	//	m_isActive = true;
	//}

	//一定の高さ以下に落ちたらデスポーンする
	if (m_objectData.pos.y <= -500.0f)
	{
		m_isActive = false;
	}
...
void C_ENEMY::MoveCalc()
{
	...
	//if (len > 72.0f)
	//{
	//	tmp = VSub(m_startPos, m_objectData.pos);
	//	len = VSize(tmp);
	//	tmp = VNorm(tmp);
	...
	//	if (len < 5)
	//	{
	//		m_objectData.pos.x = m_startPos.x;
	//		m_objectData.pos.z = m_startPos.z;
	//		m_objectData.rota.y = 0.0f;
	//		m_moveVec = { 0 };
	//	}
	//}
	...
	//{
	//	//移動処理
	//	tmp = VSub(m_targetPos, m_objectData.pos);
	//	m_objectData.rota.y = atan2f(-tmp.x, -tmp.z);
	//	tmp = VNorm(tmp);
	//	m_moveVec = VScale(tmp, ENEMY_MOVE_SPEED * m_easingSpeed);
	//	m_nowState = ENEMY_STATE_MOVE;
	//}
...
void C_ENEMY::AttackCalc()
{
	...
	////プレイヤーとの距離を計算する
	//float len = VSize(VSub(m_targetPos, m_objectData.pos));
...
void C_ENEMY::KnockBackCalc()
{
	...
	////ノックバックの移動ベクトルを計算
	//m_objectData.pos = VAdd(m_objectData.pos, tmp);
...
{
	// 二点間の距離を取得
	VECTOR vec = VSub(m_objectData.pos, _knockBackSpeed);
...
void C_ENEMY::AddPos(VECTOR _addPos)
{
	...
	//座標を加算する
	m_objectData.pos = VAdd(m_objectData.pos, _addPos);
	//移動ベクトルをリセットする
	m_moveVec.y = 0.0f;
	//落下速度をリセットする
	m_fallSpeed = 0.0f;
...
//	m_objectData.pos = m_startPos = _pos;
//	m_objectData.scale = _scale;
//	m_objectData.rota = _rotation;
//	//モデルのハンドルを複製して保存する
//	//エフェクトのハンドルを保存する
