...
int C_2D_HNDL_MANAGER::Get2DImageHndl(const char* _imgaeFilePath, int _graphNum)
{
	...
	for (auto itr = m_data.begin(); itr != m_data.end(); ++itr)
	{
		...
		if ((*itr).grapAllNum > _graphNum && _graphNum != -1)return(*itr).hndl[_graphNum];
	}
...
int C_2D_HNDL_MANAGER::Get2DImageAllNum(const char* _imgaeFilePath)
{
	//保存されている情報からファイルパスが一致するモデルハンドルを探索
	for (auto itr = m_data.begin(); itr != m_data.end(); ++itr)
	{
